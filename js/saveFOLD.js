...
function mySaveFOLD(){
    ...
    //     json.vertices_coords.push([vertex.x, vertex.y, vertex.z]);
    // }
    for (let i = 0; i < globals.model.getNodes().length; i++) {
        const node = globals.model.getNodes()[i].getOriginalPosition();
        json.vertices_coords.push([node['x'], node['y'], node['z']]);
    }
